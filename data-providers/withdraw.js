...
exports.list=async(op)=>{
    ...
    var db=await getDB();
    var cur=db.withdraw.find(op.query);
    if (op.sort) {
    ...
    var [rows, total, [sum]]=await Promise.all([cur.toArray(), cur.count(), db.withdraw.aggregate([
        {$match:op.query},
        {$group:{_id:1, amount:{$sum:'$snapshot.amount'}, fee:{$sum:'$snapshot.fee'}}}
    ]).toArray()]);
