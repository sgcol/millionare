...
<div>
	...
	<b-tabs>
		<b-tab title="管理员账号">
			<b-button-toolbar>
				<b-button-group size="sm" class="float-right" v-b-modal.modal-create>
					<b-button><b-icon-person-plus /> Create Account</b-button>
				</b-button-group>
			</b-button-toolbar>
			<b-table
				show-empty
	...
	<b-modal id="modal-create" title="新建管理员" ok-only ok-title="Create" @ok="addaccount">
		<b-form>
...
export default {
	...
	methods:{
		...
		updaccount(phone, acc) {
			var sock=openLink();
			sock.emit('$upd', {target:'users', query:{phone:phone}, content:acc}, this.showerr.bind(this))
		}
	},
	mounted() {
		this.refreshData();
